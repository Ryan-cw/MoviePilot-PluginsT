...
# 各频道过滤条件
FILTER_UI_DATA: Dict[str, List[Dict]] = {
    "tv": [
        {
            "Id": "order",
            "Text": "排序",
...
def _build_filter_ui() -> List[dict]:
    """
    构建各频道过滤 UI 组件列表
    """
    ui = []
...
class IqiyiDiscover(_PluginBase):
    ...
    # 插件图标
    plugin_icon = "https://raw.githubusercontent.com/Ryan-cw/MoviePilotPlus/main/icons/iqiyi.png"
    # 插件版本
    plugin_version = "1.0.0"
    # 插件作者
    plugin_author = "Ryan-cw"
    # 作者主页
    author_url = "https://github.com/Ryan-cw"
    # 插件配置项ID前缀
                                ...
                                "content": [
                                    {
                                        ...
                                        "props": {
                                            "type": "info",
                                            "variant": "tonal",
                                            "text": "启用后，在探索页面可切换到「爱奇艺」数据源，支持电视剧、电影、综艺、动漫、纪录片、少儿六大分类，以及排序、年份、地区、风格等多维度筛选。",
                                        },
                                    }
                                ],
                            }
    ...
    def __request(self, page: int, mtype: str, **kwargs) -> List[Dict]:
        """
        请求爱奇艺列表 API
        接口：https://mesh.if.iqiyi.com/portal/lw/videolib/data
        """
        ...
        try:
            ...
            if data.get("code") != "A00000":
                logger.error(
                    f"爱奇艺 API 错误: {data.get('code')} - {data.get('msg')}"
                )
                return []

            items = data.get("data", {}).get("list", [])
            if not items:
                logger.warning(f"爱奇艺无数据: mtype={mtype}, page={page}")
            return items

        except requests.RequestException as e:
            logger.error(f"爱奇艺请求失败: {e}")
            return []
        except (KeyError, ValueError) as e:
            logger.error(f"爱奇艺响应解析失败: {e}")
            return []
    ...
    def iqiyi_discover(
        ...
        def _to_media(item: dict) -> Optional[schemas.MediaInfo]:
            try:
                ...
                logger.warning(f"爱奇艺数据转换失败: {e}, item={item}")
                return None
        ...
        try:
            ...
            logger.error(f"爱奇艺探索异常: {e}")
            return []

        return [r for r in (_to_media(i) for i in items) if r is not None]
    ...
    def discover_source(self, event: Event):
        ...
        source = schemas.DiscoverMediaSource(
            ...
            api_path=(
                f"plugin/IqiyiDiscover/iqiyi_discover"
